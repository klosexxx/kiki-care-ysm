      ...
      <div className="max-w-7xl mx-auto px-4 py-16 grid grid-cols-1 md:grid-cols-4 gap-10">
        <div className="md:col-span-2">
          ...
          <div className="flex items-center gap-4 mt-6">
            <a href="https://vk.com/kikicare" target="_blank" rel="noopener noreferrer" className="w-9 h-9 bg-white/10 hover:bg-primary rounded-full flex items-center justify-center transition-colors">
        ...
        <div>
          ...
          <ul className="space-y-3">
            {[['/', 'главная'], ['/catalog', 'каталог'], ['/brands', 'бренды'], ['/stores', 'магазины']].map(([to, label]) => (
              <li key={to}>
                <Link to={to} className="text-gray-400 text-sm hover:text-white transition-colors lowercase tracking-wide">{label}</Link>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-3 text-gray-400 text-sm">
            <li>пл. Привокзальная, 9, Тучково</li>
            <li><a href="tel:+79378561466" className="hover:text-white transition-colors">+7 (937) 856-14-66</a></li>
            <li><a href="mailto:info@kikicare.ru" className="hover:text-white transition-colors">info@kikicare.ru</a></li>
          </ul>
        </div>
      </div>
