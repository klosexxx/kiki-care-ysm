...
// ── Beauty SVG иконки категорий ───────────────────────────

const IconFace = () => (
  <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-7 h-7">
    <ellipse cx="24" cy="22" rx="13" ry="16" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
    <path d="M18 20 Q19.5 18.5 21 20" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
    <path d="M27 20 Q28.5 18.5 30 20" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
    <path d="M24 21 L22.5 25.5 Q24 26.5 25.5 25.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M20 29 Q24 32 28 29" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
    <path d="M11 20 C11 10 37 10 37 20" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
    <path d="M11 18 C10 12 13 7 18 6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
    <path d="M37 18 C38 12 35 7 30 6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
  </svg>
)
...
const IconSerum = () => (
  <svg viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-7 h-7">
    <rect x="22" y="18" width="8" height="20" rx="4" stroke="currentColor" strokeWidth="1.8"/>
    <ellipse cx="26" cy="17" rx="6" ry="4" stroke="currentColor" strokeWidth="1.8"/>
    <path d="M26 38 L26 43" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
    <path d="M26 43 C26 43 23.5 46.5 23.5 48 C23.5 49.4 24.6 50.5 26 50.5 C27.4 50.5 28.5 49.4 28.5 48 C28.5 46.5 26 43 26 43Z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/>
    <path d="M22 28 Q26 26 30 28" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
    <circle cx="20" cy="13" r="1.5" stroke="currentColor" strokeWidth="1.2"/>
    <circle cx="33" cy="10" r="2" stroke="currentColor" strokeWidth="1.2"/>
  </svg>
)
      ...
      <div className="flex items-center justify-center gap-4 min-h-[240px]">
        {visible.map(({ Icon, title, desc, color, iconBg, offset }) => (
          ...
          >
            ...
            <div>
              <h3 className="font-heading text-xl font-semibold mb-2 lowercase tracking-tight">
                {title}
              </h3>
              {offset === 0 && (
...
export default function Home() {
      ...
      >
        <div ref={heroRef} className="absolute inset-0 will-change-transform">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-5">
            <span className="font-heading text-[30vw] font-light text-white leading-none select-none">
              KC
            </span>
          </div>
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 pb-16 w-full">
          <div className="max-w-2xl animate-fade-up">
            ...
            <div className="flex gap-4 flex-wrap">
              <Link
                to="/catalog"
                className="inline-flex items-center gap-3 bg-white text-dark text-xs tracking-[0.15em] uppercase px-8 py-4 hover:bg-primary hover:text-white transition-all duration-300"
              >
              ...
              {!user && (
                <Link
                  to="/register"
                  className="inline-flex items-center gap-3 border border-white/40 text-white text-xs tracking-[0.15em] uppercase px-8 py-4 hover:bg-white hover:text-dark transition-all duration-300"
                >
                  получить скидку
                </Link>
              )}
        ...
        <section className="py-24 px-4 bg-white">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-end justify-between mb-14">
              ...
              <Link
                to="/catalog"
                className="text-xs text-gray-400 hover:text-dark transition-colors tracking-[0.15em] uppercase flex items-center gap-2"
              >
  ...
  {categories.map((cat, i) => {
    const Icon = CATEGORY_ICONS[cat.slug] || IconSerum

    return (
      <Link
        key={cat.id}
        to={`/catalog?category=${cat.slug}`}
        className="group overflow-hidden rounded-2xl bg-[#faf8f5] border-2 border-transparent hover:border-primary/30 hover:bg-white hover:shadow-lg hover:-translate-y-1 transition-all duration-300 p-7 flex flex-col gap-5 animate-fade-up"
        style={{ animationDelay: `${i * 0.08}s`, opacity: 0 }}
      >
        {/* Иконка */}
        <div className="w-14 h-14 rounded-xl bg-white group-hover:bg-primary/10 flex items-center justify-center transition-colors duration-300 text-primary shadow-sm">
          <Icon />
        </div>

        {/* Название */}
        <p className="font-heading text-xl font-light text-dark group-hover:text-primary transition-colors duration-300 lowercase leading-tight">
          {cat.name}
        </p>
        ...
        <ArrowRight
          size={14}
          className="text-gray-200 group-hover:text-primary group-hover:translate-x-1 transition-all duration-300"
        />
      </Link>
    )
  })}
      ...
      <section className="py-28 px-4 bg-dark text-white">
        <div className="max-w-3xl mx-auto text-center">
          ...
          {!user ? (
            <Link
              to="/register"
              className="inline-flex items-center gap-3 border border-white/20 text-white text-xs tracking-[0.2em] uppercase px-10 py-4 hover:bg-white hover:text-dark transition-all duration-300"
            >
